...
def set_secret(service_client, arn, token):
    ...
    if not conn and previous_dict:
        ...
        if previous_dict['username'] != pending_dict['username']:
            logger.error(
                f"setSecret: Attempting to modify user {pending_dict['username']} other than last valid user {previous_dict['username']}"
            )

            raise ValueError(
                f"Attempting to modify user {pending_dict['username']} other than last valid user {previous_dict['username']}"
            )
    ...
    try:
        with conn.cursor() as cur:
            ...
            conn.commit()
            logger.info(
                f"setSecret: Successfully set password for user {pending_dict['username']} in MariaDB DB for secret arn {arn}."
            )
