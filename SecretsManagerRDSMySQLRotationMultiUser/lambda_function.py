...
def set_secret(service_client, arn, token):
    ...
    if conn:
        conn.close()
        logger.info(
            f"setSecret: AWSPENDING secret is already set as password in MySQL DB for secret arn {arn}."
        )
    ...
    try:
        with conn.cursor() as cur:
            ...
            for row in cur.fetchall():
                ...
                cur.execute(f"{new_grant_escaped} TO %s", (pending_dict['username'],))
            ...
            cur.execute(
                f"SET PASSWORD FOR %s = {password_option}",
                (pending_dict['username'], pending_dict['password']),
            )

            conn.commit()
            logger.info(
                f"setSecret: Successfully set password for {pending_dict['username']} in MySQL DB for secret arn {arn}."
            )
...
def get_password_option(version):
    ...
    """
    return "%s" if version.startswith("8") else "PASSWORD(%s)"
