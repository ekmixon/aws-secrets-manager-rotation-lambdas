...
def set_secret(service_client, arn, token):
    ...
    if get_alt_username(current_dict['username']) != pending_dict['username']:
        logger.error(
            f"setSecret: Attempting to modify user {pending_dict['username']} other than current user clone {get_alt_username(current_dict['username'])}"
        )

        raise ValueError(
            f"Attempting to modify user {pending_dict['username']} other than current user clone {get_alt_username(current_dict['username'])}"
        )
    ...
    try:
        with conn.cursor() as cur:
            ...
            if len(cur.fetchall()) == 0:
                create_role = f"CREATE ROLE {pending_username}"
                cur.execute(
                    f"{create_role} WITH LOGIN PASSWORD %s",
                    (pending_dict['password'],),
                )

                cur.execute(f"GRANT {current_username} TO {pending_username}")
            else:
