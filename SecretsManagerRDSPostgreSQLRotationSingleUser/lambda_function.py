...
def set_secret(service_client, arn, token):
    ...
    if not conn and previous_dict:
        ...
        if previous_dict['host'] != pending_dict['host']:
            logger.error(
                f"setSecret: Attempting to modify user for host {pending_dict['host']} other than previous valid host {previous_dict['host']}"
            )

            raise ValueError(
                f"Attempting to modify user for host {pending_dict['host']} other than current previous valid {previous_dict['host']}"
            )
