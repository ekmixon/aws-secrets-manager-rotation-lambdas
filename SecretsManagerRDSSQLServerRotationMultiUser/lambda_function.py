...
def set_password_for_login(cursor, current_db, current_login, pending_dict):
    ...
    if len(cursor.fetchall()) == 0:
        ...
        create_login = f"CREATE LOGIN {escaped_pending_username}"
        ...
        if current_db == 'master':
            ...
            for row in cursor.fetchall():
                if row['state_desc'] == 'GRANT_WITH_GRANT_OPTION':
                    cursor.execute(
                        f"GRANT {row['permission_name']} TO {escaped_pending_username} WITH GRANT OPTION"
                    )

                else:
                    cursor.execute(
                        f"{row['state_desc']} {row['permission_name']} TO {escaped_pending_username}"
                    )
            ...
            if len(cursor.fetchall()) == 0:
                ...
                cursor.execute(
                    f"CREATE USER {escaped_alt_username} FOR LOGIN {escaped_pending_username}"
                )
        ...
        alter_stmt = f"ALTER LOGIN {escaped_pending_username}"
...
def set_password_for_user(cursor, current_user, pending_dict):
    ...
    if len(cursor.fetchall()) == 0:
        # Create the new user
        create_login = f"CREATE USER {escaped_pending_username}"
        cursor.execute(f"{create_login} WITH PASSWORD = %s", pending_dict['password'])

        apply_database_permissions(cursor, current_user, escaped_pending_username)
    else:
        alter_stmt = f"ALTER USER {escaped_pending_username}"
        cursor.execute(f"{alter_stmt} WITH PASSWORD = %s", pending_dict['password'])
