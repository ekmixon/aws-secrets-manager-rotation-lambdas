...
def set_secret(service_client, arn, token):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if containment == 0:
                alter_stmt = f"ALTER LOGIN {escaped_username}"
            else:
                alter_stmt = f"ALTER USER {escaped_username}"
            cursor.execute(
                f"{alter_stmt} WITH PASSWORD = %s OLD_PASSWORD = %s",
                (pending_dict['password'], current_dict['password']),
            )

            conn.commit()
            logger.info(
                f"setSecret: Successfully set password for user {pending_dict['username']} in SQL Server DB for secret arn {arn}."
            )
